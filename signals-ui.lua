...
local function open_prometheus_combinator_gui(player, entity)
    ...
    signal_group_container.add { type = "textfield", name = "group-name", text = stored_group }

    signal_group_container.enabled = enable_signal_groups
    -- only disable signal_group_container to retain group-information even when disabled
...
local function apply_prometheus_combinator_gui_inputs(frame, entity)
    ...
    if existing_combinator_table then
...
local function find_own_frame(element)
    if element.parent then
        ...
        elseif element.parent.parent then
            if element.parent.parent.name == "prometheus-combinator-gui" then
                return element.parent.parent
            elseif element.parent.parent.parent then
...
function on_signals_gui_checked_state_changed(event)
    local frame = find_own_frame(event.element)
    if not frame then
        return
    end
    if event.element.name ~= "all-signals-toggle" then
        return
    end
    frame["specific-signal-container"].visible = not event.element.state
    validate_prometheus_combinator_gui_inputs(frame)
end
...
function on_signals_gui_closed(event)
    local element = event.element
    if element and element.name == "prometheus-combinator-gui" then
        element.destroy()
    end
end
...
function on_signals_gui_opened(event)
    if event.entity and event.entity.name == "prometheus-combinator" then
...
function add_signals_ui_event_handlers()
    script.on_event(defines.events.on_gui_checked_state_changed, on_signals_gui_checked_state_changed)
